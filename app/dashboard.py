...
st.write("""
         ...
         interpolation between all pairs of conformations (20k points). These datasets also include a
         broad range of quantum chemical properties calculated with density functional theory.
         """)
...
def data_to_plot(property):
    ...
    elif property == 'Zero-point energy':
       zpe = data['ZPE'].flatten()
       df = pd.DataFrame({'Zero-point energy': zpe})
    elif property == 'Vibrational frequencies':
       df = process_freq_data(data)
    elif property == 'Electronic + thermal':
       df = process_thermal_data(data)
...
properties = ('Potential energy', 'Forces', 'Mulliken charges', 'Dipole moment', 'Quadrupole moment',
              'Polarizability', 'HOMO-LUMO gap', 'Zero-point energy', 'Vibrational frequencies',
              'Electronic + thermal', 'Electronic spatial extent')
